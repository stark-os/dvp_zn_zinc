...
#main
def c05_unusedFcts(zCtx):
	zCtx.updateLogLvl(STEP.C05)
	zCtx.dbgSepLine()
	zCtx.dbg0("=================================================================================")
	zCtx.dbg0("========================== C05 UNUSED FCTS : beginning ==========================")
	zCtx.dbg0("=================================================================================")
	zCtx.dbgPause()
	...
	if log_lvl[0] >= LOG__LVL_DBG0:
		...
		output = \
			"FCTS REMAINING (used):\n\n" + '\n'.join(calledFcts) + \
			"\n\n\n\nFCTS REMOVED (unused):\n\n" + '\n'.join(rmFctNames) + '\n'
		writeFile("dbg/" + path_name(zCtx.initialCtx.filename) + ".c05.lst", output)
